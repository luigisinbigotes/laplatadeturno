...
const FALLBACK_MAP_BASE = "https://www.google.com/maps/dir/?api=1";
  ...
  return rows
    .map((row) => {
      ...
      return {
        ...
        zone: zone ?? "La Plata",
        phone: phone ?? "",
        mapUrl: sanitizeMapUrl(mapUrl, destination),
...
function sanitizeMapUrl(mapUrl, destinationMatch) {
  try {
  ...
  if (destinationMatch) {
    const fallbackUrl = new URL(FALLBACK_MAP_BASE);
    fallbackUrl.searchParams.set("destination", `${destinationMatch[1]},${destinationMatch[2]}`);
...
export function withNullDistances(pharmacies) {
  return pharmacies.map((pharmacy) => ({
    ...pharmacy,
    distanceKm: null
  }));
